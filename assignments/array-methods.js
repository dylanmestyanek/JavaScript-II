...
let firstNamesAllCaps = runners.map(runner => runner.first_name.replace(runner.first_name[0], runner.first_name[0].toUpperCase()));
...
runners.reduce((total, runner) => ticketPriceTotal += runner.donation, 0);
...
// Problem 1
// Return runners with an even ID value
let evenIdRunners = runners.filter(runner => runner.id % 2 == 0);
console.log(evenIdRunners);

// Problem 2
// Sort runners alphabetically by last name
let sortedRunners = runners.sort((a, b) => a.last_name > b.last_name ? 1 : -1);
console.log(sortedRunners);

// Problem 3
// Return only the runners in the Skinix company
